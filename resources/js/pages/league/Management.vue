...
type ModulePayload = {
    ...
    cut_selector: {
        ...
        cuts: Array<{ id: number; label: string; is_active: boolean }>;
    };
    summary: {
        selected_cut: {
            id: number;
    ...
    expenses: ExpenseRow[];
    board: {
        members: Array<{ id: number; name: string; share_cents: number }>;
        share_cents: number;
    };
    settings: {
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Gestión', href: '/liga/gestion' },
];
...
const settingsForm = reactive({
    ...
    incoming_team_guest_limit: props.module.settings.incoming_team_guest_limit,
    member_fee_amount_cents:
        props.module.settings.member_fee_amount_cents / 100,
    guest_fee_amount_cents: props.module.settings.guest_fee_amount_cents / 100,
...
const filteredPayments = computed(() =>
    paymentFilter.value === 'all'
        ...
        : props.module.payments.filter(
              (payment) => payment.balance.status !== 'paid',
          ),
);

function onCutChange(event: Event): void {
    const target = event.target as HTMLSelectElement;
    router.get(
        '/liga/gestion',
        { cut_id: Number(target.value) },
        { preserveScroll: true, preserveState: true },
    );
}
function openPaymentModal(payment: PaymentRow): void {
    selectedPayment.value = payment;
    paymentForm.amount_cents = String(
...
return;
    ...
    router.post(
        `/liga/gestion/payments/${selectedPayment.value.player.id}`,
        {
            cut_id: props.module.cut_selector.selected_cut_id,
            amount_cents: paymentForm.amount_cents
                ? Math.round(Number(paymentForm.amount_cents) * 100)
                : 0,
...
return;
    ...
    router.delete(`/liga/gestion/payments/${selectedPayment.value.player.id}`, {
        data: { cut_id: props.module.cut_selector.selected_cut_id },
        preserveScroll: true,
        onSuccess: () => {
            selectedPayment.value = null;
        },
...
function submitExpense(): void {
    if (!expenseForm.name.trim() || !expenseForm.amount_cents) {
return;
}

    router.post(
        '/liga/gestion/expenses',
        {
            cut_id: props.module.cut_selector.selected_cut_id,
            name: expenseForm.name,
            amount_cents: Math.round(Number(expenseForm.amount_cents) * 100),
...
function deleteExpense(expenseId: number): void {
    router.delete(`/liga/gestion/expenses/${expenseId}`, {
        preserveScroll: true,
    });
}
function submitReferral(): void {
    if (!referralForm.referrer_player_id || !referralForm.referred_player_id) {
return;
}

    router.post(
        '/liga/gestion/referrals',
        {
...
function deleteReferral(referralId: number): void {
    router.delete(`/liga/gestion/referrals/${referralId}`, {
        preserveScroll: true,
    });
}
    ...
    router.post(
        '/liga/gestion/settings',
        {
            ...
            member_fee_amount_cents: Math.round(
                settingsForm.member_fee_amount_cents * 100,
            ),
            guest_fee_amount_cents: Math.round(
                settingsForm.guest_fee_amount_cents * 100,
            ),
            referral_credit_amount_cents: Math.round(
                settingsForm.referral_credit_amount_cents * 100,
            ),
        },
        { preserveScroll: true },
    );
...
function downloadReport(): void {
    window.open(
        `/liga/gestion/report?cut_id=${props.module.cut_selector.selected_cut_id}`,
        '_blank',
        'noopener,noreferrer',
    );
}
...
<template>
    ...
    <LeagueShellLayout
        ...
        active-module="gestion"
        :can-manage-league="true"
    >
        <section class="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_320px]">
            <article class="app-surface space-y-5">
                <div class="flex items-start justify-between gap-4">
                    <div class="space-y-3">
                        ...
                        <h1 class="app-display app-module-title text-[#F8FAFC]">
                            {{ props.module.summary.selected_cut.label }}
                        </h1>
                    </div>
                    <select
                        :value="props.module.cut_selector.selected_cut_id"
                        class="min-h-12 rounded-[12px] border border-white/8 bg-[#0E1628] px-4 text-sm text-[#F8FAFC] outline-none"
                        @change="onCutChange"
                    >
                        <option
                            v-for="cut in props.module.cut_selector.cuts"
                            :key="cut.id"
                            :value="cut.id"
                        >
                            {{ cut.label
                            }}{{ cut.is_active ? ' · activo' : '' }}
                        </option>
                    </select>
                </div>
                <div class="grid gap-3 md:grid-cols-3">
                    ...
                    >
                        <p class="app-kicker">Entradas</p>
                        <p
                            class="mt-3 text-[24px] font-semibold text-[#4ADE80]"
                        >
                            {{
                                formatMoney(
                                    props.module.summary.income.total_cents,
                                )
                            }}
                        </p>
                    </div>
                    ...
                    >
                        ...
                        <p
                            class="mt-3 text-[24px] font-semibold text-[#FCA5A5]"
                        >
                            {{
                                formatMoney(
                                    props.module.summary.expenses.total_cents,
                                )
                            }}
                        </p>
                    </div>
                    <div
                        class="rounded-[16px] border border-white/6 bg-[#0E1628] p-4"
                    >
                        <p class="app-kicker">Balance</p>
                        <p
                            class="mt-3 text-[24px] font-semibold text-[#E5B849]"
                        >
                            {{
                                formatMoney(props.module.summary.balance_cents)
                            }}
                        </p>
                    </div>
                </div>
            ...
            <article class="app-surface space-y-3">
                ...
                <Button
                    ...
                    ><Download class="mr-2 size-4" />Guardar reporte PDF</Button
                >
            </article>
        </section>
        ...
        <section class="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_340px]">
                ...
                <div
                    ...
                    :class="
                        filteredPayments.length > 10
                            ? 'max-h-[680px] overflow-y-auto pr-1'
                            : ''
                    "
                >
                    <button
                        ...
                        type="button"
                        class="w-full rounded-[16px] border border-white/6 bg-[#0E1628] p-4 text-left transition hover:border-[rgba(229,184,73,0.24)]"
                        @click="openPaymentModal(payment)"
                    >
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex min-w-0 items-start gap-3">
                                <div
                                    class="mt-1 flex size-10 shrink-0 items-center justify-center rounded-full border border-white/6 bg-[#131B2F]"
                                >
                                    <CheckCircle2
                                        v-if="payment.balance.status === 'paid'"
                                        class="size-4 text-[#4ADE80]"
                                    /><CircleAlert
                                        v-else
                                ...
                                <div class="min-w-0">
                                    ...
                                    >
                                        {{ payment.balance.status_message }}
                                    </p>
            ...
            <div class="grid gap-4">
                <article class="app-surface space-y-4">
                    ...
                    <div class="grid gap-3">
                        ...
                        >
                            ...
                            <p
                                class="mt-3 text-[18px] font-semibold text-[#4ADE80]"
                            >
                                {{
                                    formatMoney(
                                        props.module.summary.income
                                            .cash_payments_cents,
                                    )
                                }}
                            </p>
                ...
                <article class="app-surface space-y-4">
                    <div class="flex items-center gap-3">
                        <Landmark class="size-5 text-[#E5B849]" />
                        <p class="app-kicker text-[#E5B849]">Directiva</p>
                    </div>
                    <div class="grid gap-3 sm:grid-cols-2">
                        <div
                            v-for="member in props.module.board.members"
                            :key="member.id"
                            class="rounded-[14px] border border-white/6 bg-[#0E1628] p-4"
                        >
                            <p class="text-sm font-semibold text-[#F8FAFC]">
                            ...
                            <p
                                class="mt-3 text-[18px] font-semibold text-[#E5B849]"
                            >
                                {{ formatMoney(member.share_cents) }}
                            </p>
                        </div>
                    </div>
                </article>
        ...
        <section class="grid gap-4 xl:grid-cols-2">
            <article class="app-surface space-y-4">
                <div class="flex items-center gap-3">
                    <Receipt class="size-5 text-[#E5B849]" />
                    <p class="app-kicker text-[#E5B849]">Gastos del corte</p>
                </div>
                <div class="grid gap-3">
                    <div
                        v-for="expense in props.module.expenses"
                        :key="expense.id"
                        class="rounded-[14px] border border-white/6 bg-[#0E1628] p-4"
                    >
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <div class="text-right">
                                <p
                                    class="text-[16px] font-semibold text-[#F8FAFC]"
                                >
                                    {{ formatMoney(expense.amount_cents) }}
                                </p>
                                <button
                                    v-if="!expense.is_system_generated"
                                    type="button"
                                    class="mt-2 text-[12px] font-semibold text-[#FCA5A5]"
                ...
                <div
                    class="grid gap-3 rounded-[16px] border border-white/6 bg-[#0E1628] p-4"
                >
                    <input
                    ...
                    />
                    <label
                        class="flex items-center gap-3 rounded-[12px] border border-white/6 bg-[#131B2F] px-4 py-3 text-sm text-[#F8FAFC]"
                        ><input
                            v-model="expenseForm.is_fixed"
                            type="checkbox"
                            class="size-4 rounded border-white/10 bg-[#0E1628]"
                        />Marcar como gasto fijo</label
                    >
                    <Button
                        type="button"
                        variant="secondary"
            ...
            <article class="app-surface space-y-4">
                <div class="flex items-center gap-3">
                    ...
                    <p class="app-kicker text-[#E5B849]">Referidos</p>
                </div>
                <div
                    v-if="props.module.referrals.length === 0"
                    class="rounded-[14px] border border-dashed border-white/8 bg-[#0E1628] p-4 text-sm text-[#94A3B8]"
                >
                    Sin referidos registrados.
                </div>
                <div
                    v-for="group in props.module.referrals"
                ...
                >
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <p class="text-[12px] font-semibold text-[#E5B849]">
                            {{ formatMoney(group.available_credit_cents) }}
                        </p>
                    </div>
                    <div class="mt-3 flex flex-wrap gap-2">
                        <button
                            ...
                            :key="member.id"
                            type="button"
                            class="rounded-full border border-[rgba(248,113,113,0.28)] bg-[rgba(248,113,113,0.12)] px-3 py-1 text-[11px] font-semibold text-[#FCA5A5]"
                            @click="deleteReferral(member.id)"
                        >
                            Quitar {{ member.name }}
                        </button>
                    </div>
                ...
                >
                    <select
                        v-model="referralForm.referrer_player_id"
                        class="min-h-12 rounded-[12px] border border-white/8 bg-[#131B2F] px-4 text-sm text-[#F8FAFC] outline-none"
                    >
                        <option value="">Referidor</option>
                        <option
                            v-for="player in props.module.roster_management
                                .referral_options"
                            :key="player.id"
                            :value="player.id"
                        >
                    ...
                    <select
                        v-model="referralForm.referred_player_id"
                        class="min-h-12 rounded-[12px] border border-white/8 bg-[#131B2F] px-4 text-sm text-[#F8FAFC] outline-none"
                    >
        ...
        <section class="grid gap-4 xl:grid-cols-2">
            <article class="app-surface space-y-4">
                <div class="flex items-center gap-3">
                    <Settings2 class="size-5 text-[#E5B849]" />
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    <label class="grid gap-2 text-sm text-[#F8FAFC]"
                        ...
                        ><input
                            v-model.number="settingsForm.guest_fee_amount_cents"
                            type="number"
                            min="1"
                            class="min-h-12 rounded-[12px] border border-white/8 bg-[#0E1628] px-4 text-sm text-[#F8FAFC] outline-none"
                    /></label>
                    <label
                        class="grid gap-2 text-sm text-[#F8FAFC] sm:col-span-2"
                        ><span>Límite de invitados por equipo nuevo</span
                        ><input
                            v-model.number="
                    ...
                    /></label>
                    <p
                        class="text-[12px] leading-6 text-[#94A3B8] sm:col-span-2"
                    >
                        Este tope se aplica cada vez que entra un equipo nuevo
                        desde la cola. Si entran dos equipos nuevos por una
                        rotación doble, el sistema duplica automáticamente el
                        límite configurado.
                    </p>
                    <label
                        class="grid gap-2 text-sm text-[#F8FAFC] sm:col-span-2"
                        ><span>Crédito por referido</span
                        ><input
                            v-model.number="
                                settingsForm.referral_credit_amount_cents
                            "
                            type="number"
                            min="1"
                            class="min-h-12 rounded-[12px] border border-white/8 bg-[#0E1628] px-4 text-sm text-[#F8FAFC] outline-none"
                ...
                <div class="flex flex-wrap gap-2">
                    <button
                        v-for="day in gameDayOptions"
                        :key="day"
                        type="button"
                        class="min-h-10 rounded-full border px-3 text-xs font-semibold"
                        :class="
                            settingsForm.game_days.includes(day)
                                ? 'border-[rgba(229,184,73,0.28)] bg-[rgba(229,184,73,0.12)] text-[#F8FAFC]'
                                : 'border-white/6 bg-[#131B2F] text-[#94A3B8]'
                        "
                        @click="toggleGameDay(day)"
                    >
                        {{ day }}
                    </button>
                </div>
            ...
            <article class="app-surface space-y-4">
                <div class="flex items-center gap-3">
                    <Wallet class="size-5 text-[#E5B849]" />
                    <p class="app-kicker text-[#E5B849]">Balance de la liga</p>
                </div>
                <div class="grid gap-3">
                    <div
                        class="rounded-[14px] border border-white/6 bg-[#0E1628] p-4"
                ...
                <div v-if="selectedPayment" class="grid gap-3">
                    <div class="grid gap-3 sm:grid-cols-3">
                        ...
                        >
                            ...
                            <p
                                class="mt-2 text-[18px] font-semibold text-[#F8FAFC]"
                            >
                                {{
                                    formatMoney(
                        ...
                        >
                            ...
                            <p
                                class="mt-2 text-[18px] font-semibold text-[#E5B849]"
                            >
                                {{
                                    formatMoney(
                                        selectedPayment.balance
                    ...
                    <input
                        v-model="paymentForm.amount_cents"
                        type="number"
                        min="0"
                        class="min-h-12 rounded-[12px] border border-white/8 bg-[#131B2F] px-4 text-sm text-[#F8FAFC] outline-none"
                        placeholder="Monto en pesos"
                    ...
                    <label
                        class="flex items-center gap-3 rounded-[12px] border border-white/6 bg-[#131B2F] px-4 py-3 text-sm text-[#F8FAFC]"
                        ><input
                            v-model="paymentForm.apply_referral_credit"
                            type="checkbox"
                            class="size-4 rounded border-white/10 bg-[#0E1628]"
                        />Aplicar crédito disponible ({{
                            formatMoney(
                                selectedPayment.balance
                                    .available_referral_credit_cents,
                            )
                        }})</label
                    >
                    <p
                        v-if="selectedPayment.balance.previous_debt_cents > 0"
                        class="rounded-[12px] border border-[rgba(248,113,113,0.28)] bg-[rgba(248,113,113,0.12)] px-4 py-3 text-[13px] text-[#FCA5A5]"
                    >
                        Tiene deuda anterior por
                        {{
                            formatMoney(
                                selectedPayment.balance.previous_debt_cents,
                            )
                        }}.
                    </p>
                </div>
