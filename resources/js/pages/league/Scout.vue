...
const ratingFields: Array<{ key: RatingField; label: string; icon: typeof Zap }> = [
  ...
  { key: 'scoring_rating', label: 'Anotación', icon: Sparkles },
...
return `${row.stat_rating.detail.diversity}% balance`
  ...
  if (key === 'defense_rating') {
return row.stat_rating.detail.points_allowed_per_game === null ? 'Sin defensa' : `${row.stat_rating.detail.points_allowed_per_game.toFixed(1)} pts recibidos`
}
...
<template>
  ...
  <LeagueShellLayout :breadcrumbs="breadcrumbs" :league-name="props.league.name" :league-emoji="props.league.emoji" :role-label="props.role.label" active-module="scout" :can-manage-league="props.role.can_manage">
    <section class="app-surface space-y-4">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="space-y-3">
          ...
          <h1 class="app-display app-module-title text-[#F8FAFC]">Rating híbrido y draft automático</h1>
          <p class="text-[14px] leading-7 text-[#94A3B8]">El ranking, la vista previa y el auto draft usan exactamente la misma combinación entre percepción manual y estadísticas reales de temporada.</p>
        </div>
    ...
    <section class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
            ...
            <div class="flex items-center justify-between gap-3">
              <div class="min-w-0">
                <div class="flex items-center gap-3">
                  ...
                  <div><p class="truncate text-[15px] font-semibold text-[#F8FAFC]">{{ row.player.name }}</p><p class="mt-1 text-[12px] text-[#94A3B8]">{{ row.profile.position || 'Sin posición' }} / {{ row.profile.role || 'Sin rol' }}</p></div>
                </div>
              </div>
      ...
      <article class="app-surface space-y-4">
        <div class="flex items-center justify-between gap-3">
          <div><p class="app-kicker text-[#E5B849]">Auto balance</p><p class="mt-2 text-[13px] leading-6 text-[#94A3B8]">Vista previa del mismo motor automático que usa Juego cuando el pool actual está listo.</p></div>
    ...
    <section class="app-surface space-y-4">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div><p class="app-kicker text-[#E5B849]">Perfiles individuales</p><p class="mt-2 text-[13px] leading-6 text-[#94A3B8]">Los admins ajustan estrellas, posición, rol y consistencia. Los miembros solo ven la información.</p></div>
      ...
      <div v-if="filteredPlayers.length === 0" class="rounded-[14px] border border-dashed border-white/8 bg-[#0E1628] p-4 text-sm text-[#94A3B8]">No hay jugadores que coincidan con la búsqueda.</div>
      <div v-else class="grid gap-4">
        <article v-for="row in filteredPlayers" :key="row.player.id" class="rounded-[20px] border border-white/6 bg-[#0E1628] p-4">
          ...
          <div v-if="row.stat_rating" class="mt-4 rounded-[16px] border border-[rgba(229,184,73,0.16)] bg-[#131B2F] p-4">
            <div class="flex items-center justify-between gap-3"><div><p class="app-kicker text-[#E5B849]">Rating de temporada</p><p class="mt-2 text-[13px] leading-6 text-[#94A3B8]">Calculado con victorias, anotación, defensa, triples y diversidad real de tiros.</p></div><span class="app-display text-[28px] text-[#E5B849]">{{ row.stat_rating.overall.toFixed(1) }}</span></div>
            <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              <div class="rounded-[14px] border border-white/6 bg-[#0E1628] p-3"><p class="text-[11px] uppercase tracking-[0.18em] text-[#94A3B8]">Anotación</p><p class="mt-2 font-['Bebas_Neue'] text-[28px] leading-none text-[#4ADE80]">{{ row.stat_rating.scoring.toFixed(1) }}</p><p class="mt-2 text-[11px] text-[#94A3B8]">{{ row.stat_rating.detail.points_per_game.toFixed(1) }} pts/j</p></div>
