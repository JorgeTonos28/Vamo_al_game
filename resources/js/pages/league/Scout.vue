...
const filteredPlayers = computed(() => {
    ...
    if (!query) {
        return props.scout.players;
    }

    return props.scout.players.filter((row) => row.player.name.toLowerCase().includes(query));
});
...
const tipMap: Record<string, ScoutTip> = {
    ...
    rating: {
        ...
        tag: 'HÍBRIDO — EVOLUCIONA CON JORNADAS',
        tone: 'hybrid',
    },
};
...
const ratingFields: Array<{
    ...
    { key: 'dribbling_rating', label: 'Dribbling', icon: Target },
    { key: 'scoring_rating', label: 'Anotacion', icon: Sparkles },
    { key: 'team_play_rating', label: 'Juego en equipo', icon: Users },
...
function patchScout(row: ScoutRow, updates: Partial<ScoutProfile>) {
    ...
    router.patch(
        `/liga/modulos/scout/players/${row.player.id}`,
        {
            position: updates.position ?? row.profile.position ?? null,
            role: updates.role ?? row.profile.role ?? null,
            offensive_consistency:
                updates.offensive_consistency ?? row.profile.offensive_consistency ?? null,
            ...
            court_knowledge_rating:
                updates.court_knowledge_rating ?? row.profile.court_knowledge_rating,
...
function toggleChoice(
    ...
    const current = row.profile[key];
    patchScout(row, { [key]: current === value ? null : value } as Partial<ScoutProfile>);
}

function toggleRating(row: ScoutRow, key: RatingField, value: number) {
    const current = row.profile[key];
    patchScout(row, { [key]: current === value ? 0 : value } as Partial<ScoutProfile>);
}

function tipToneClass(tone: ScoutTip['tone']): string {
    return tone === 'manual'
        ? 'border-[rgba(167,139,250,0.28)] bg-[rgba(167,139,250,0.16)] text-[#C4B5FD]'
        : tone === 'hybrid'
          ? 'border-[rgba(74,222,128,0.28)] bg-[rgba(74,222,128,0.16)] text-[#4ADE80]'
          : 'border-[rgba(229,184,73,0.28)] bg-[rgba(229,184,73,0.16)] text-[#E5B849]';
}
</script>
    ...
    >
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="space-y-3">
                    <div class="flex items-center gap-3">
                        <Search class="size-5 text-[#E5B849]" />
                        <p class="app-kicker text-[#E5B849]">Scout de liga</p>
                    </div>
                    <h1 class="app-display app-module-title text-[#F8FAFC]">
                        Perfiles y balance automatico
                    </h1>
                    <p class="text-[14px] leading-7 text-[#94A3B8]">
                        Este modulo define el reparto automatico del draft inicial y mezcla percepcion manual con estadisticas reales de temporada.
                    </p>
                </div>
        ...
        <section class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
            <article class="app-surface space-y-4">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p class="app-kicker text-[#E5B849]">Ranking de calidad</p>
                        <p class="mt-2 text-[13px] leading-6 text-[#94A3B8]">
                            Ordenado por rating combinado. El reparto automatico usa esta base para intentar balancear ambos equipos.
                        </p>
                    </div>
                    <span class="rounded-full border border-white/6 bg-[#0E1628] px-3 py-1 text-[12px] text-[#94A3B8]">
                        {{ props.scout.ranking.length }} jugadores
                    </span>
                </div>

                <div
                    v-if="props.scout.ranking.length === 0"
                    class="rounded-[14px] border border-dashed border-white/8 bg-[#0E1628] p-4 text-sm text-[#94A3B8]"
                >
                    Todavia no hay perfiles para ordenar.
                </div>

                <div v-else class="grid gap-3">
                    <div
                        v-for="(row, index) in props.scout.ranking.slice(0, 8)"
                        :key="row.player.id"
                    ...
                    >
                        <div class="flex items-center justify-between gap-3">
                            <div class="min-w-0">
                                <div class="flex items-center gap-3">
                                    <span class="inline-flex min-w-10 items-center justify-center rounded-full border border-white/8 bg-[#131B2F] px-3 py-2 text-xs font-semibold text-[#F8FAFC]">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-[12px] text-[#94A3B8]">
                                            {{ row.profile.position || 'Sin posicion' }} · {{ row.profile.role || 'Sin rol' }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="app-display text-[26px] text-[#E5B849]">
                                    {{ row.combined_rating > 0 ? row.combined_rating.toFixed(1) : '0.0' }}
                                </p>
                                <p class="text-[11px] text-[#94A3B8]">
                                    {{ row.has_stats ? 'manual + stats' : 'solo manual' }}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <article class="app-surface space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <p class="app-kicker text-[#E5B849]">Auto balance</p>
                        <p class="mt-2 text-[13px] leading-6 text-[#94A3B8]">
                            Vista previa del mismo modo automatico que usa Juego cuando el pool actual esta listo.
                        </p>
                    </div>
                    <button
                        type="button"
                        class="inline-flex size-9 items-center justify-center rounded-full border border-white/8 bg-[#131B2F] text-[#94A3B8]"
                        @click="openTip = tipMap.rating"
                    >
                ...
                <div
                    v-if="!props.scout.auto_preview"
                    class="rounded-[14px] border border-dashed border-white/8 bg-[#0E1628] p-4 text-sm text-[#94A3B8]"
                >
                    La vista previa se activa cuando Llegada deja exactamente 10 jugadores listos en el pool inicial.
                </div>

                <template v-else>
                    <div class="grid gap-3 md:grid-cols-2">
                        <div class="rounded-[16px] border border-[rgba(74,222,128,0.22)] bg-[rgba(74,222,128,0.08)] p-4">
                            <div class="flex items-center justify-between gap-3">
                                <p class="app-kicker text-[#4ADE80]">Equipo A</p>
                                <span class="text-sm font-semibold text-[#4ADE80]">{{ props.scout.auto_preview.team_a_rating.toFixed(1) }}</span>
                            </div>
                            <div class="mt-3 grid gap-2">
                                <div v-for="player in props.scout.auto_preview.team_a" :key="player.id" class="rounded-[12px] border border-white/6 bg-[#0E1628] px-3 py-2">
                                    <p class="text-[13px] font-semibold text-[#F8FAFC]">{{ player.name }}</p>
                                    <p class="mt-1 text-[11px] text-[#94A3B8]">
                                        {{ player.position || 'Sin posicion' }} · {{ player.role || 'Sin rol' }} · {{ player.combined_rating.toFixed(1) }}
                                    </p>
                                </div>
                            </div>
                        </div>

                        <div class="rounded-[16px] border border-[rgba(229,184,73,0.22)] bg-[rgba(229,184,73,0.08)] p-4">
                            <div class="flex items-center justify-between gap-3">
                                <p class="app-kicker text-[#E5B849]">Equipo B</p>
                                <span class="text-sm font-semibold text-[#E5B849]">{{ props.scout.auto_preview.team_b_rating.toFixed(1) }}</span>
                            </div>
                            <div class="mt-3 grid gap-2">
                                <div v-for="player in props.scout.auto_preview.team_b" :key="player.id" class="rounded-[12px] border border-white/6 bg-[#0E1628] px-3 py-2">
                                    <p class="text-[13px] font-semibold text-[#F8FAFC]">{{ player.name }}</p>
                                    <p class="mt-1 text-[11px] text-[#94A3B8]">
                                        {{ player.position || 'Sin posicion' }} · {{ player.role || 'Sin rol' }} · {{ player.combined_rating.toFixed(1) }}
                                    </p>
        ...
        <section class="app-surface space-y-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    <p class="app-kicker text-[#E5B849]">Perfiles individuales</p>
                    <p class="mt-2 text-[13px] leading-6 text-[#94A3B8]">
                        Los admins ajustan estrellas, posicion, rol y consistencia. Los miembros solo ven la informacion.
                    </p>
                </div>
                <label class="relative block w-full md:max-w-[280px]">
                    <Search class="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#64748B]" />
                    <input
                        v-model="searchTerm"
                        type="text"
                        placeholder="Buscar jugador"
                        class="min-h-12 w-full rounded-[14px] border border-white/8 bg-[#0E1628] pl-10 pr-4 text-sm text-[#F8FAFC] outline-none placeholder:text-[#64748B]"
                    >
                </label>
            </div>
            ...
            <div v-else class="grid gap-4">
                <article
                    ...
                    :key="row.player.id"
                    class="rounded-[20px] border border-white/6 bg-[#0E1628] p-4"
                >
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div class="space-y-2">
                            <div class="flex items-center gap-3">
                                <div class="flex size-11 items-center justify-center rounded-full border border-[rgba(229,184,73,0.22)] bg-[#131B2F] text-sm font-semibold text-[#E5B849]">
                                    {{ row.player.jersey_number ?? 'SC' }}
                                </div>
                                <div>
                                    <p class="text-[16px] font-semibold text-[#F8FAFC]">{{ row.player.name }}</p>
                                    <p class="mt-1 text-[12px] text-[#94A3B8]">
                                        Rating {{ row.combined_rating > 0 ? row.combined_rating.toFixed(1) : '0.0' }} · Manual {{ row.manual_rating.toFixed(1) }} · Stats {{ row.stat_rating?.toFixed(1) ?? 'n/a' }}
                                    </p>
                                </div>
                            </div>
                            <p class="text-[12px] leading-6 text-[#94A3B8]">
                                <template v-if="row.season_stats">
                                    {{ row.season_stats.games }} juegos · {{ row.season_stats.points }} puntos · {{ row.season_stats.win_rate }}% victorias · {{ row.season_stats.sessions_attended }} jornadas.
                                </template>
                                <template v-else>
                                    Aun no tiene suficiente estadistica acumulada; el peso actual es mayormente manual.
                                </template>
                            </p>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                ...
                                :class="row.has_stats ? 'border-[rgba(229,184,73,0.28)] bg-[rgba(229,184,73,0.12)] text-[#E5B849]' : 'border-white/8 bg-[#131B2F] text-[#94A3B8]'"
                            >
                                {{ row.has_stats ? 'manual + stats' : 'solo manual' }}
                            </span>
                        </div>
                    ...
                    <div class="mt-4 grid gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                        <div class="grid gap-4">
                            <div class="rounded-[16px] border border-white/6 bg-[#131B2F] p-4">
                                <div class="mb-3 flex items-center gap-2">
                                    <Tag class="size-4 text-[#E5B849]" />
                                    <p class="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#94A3B8]">
                                        Posicion
                                    </p>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    <button
                                        v-for="option in props.scout.meta.positions"
                                        :key="option"
                                        type="button"
                                        class="min-h-10 rounded-full border px-4 text-xs font-semibold"
                                        :class="row.profile.position === option ? 'border-[rgba(229,184,73,0.28)] bg-[rgba(229,184,73,0.12)] text-[#F8FAFC]' : 'border-white/6 bg-[#0E1628] text-[#94A3B8]'"
                                        :disabled="!canManage"
                                        @click="toggleChoice(row, 'position', option)"
                                    >
                                        {{ option }}
                                    </button>
                                </div>
                            ...
                            <div class="rounded-[16px] border border-white/6 bg-[#131B2F] p-4">
                                <div class="mb-3 flex items-center justify-between gap-3">
                                    <div class="flex items-center gap-2">
                                        <Users class="size-4 text-[#E5B849]" />
                                        <p class="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#94A3B8]">
                                            Rol
                                        </p>
                                    </div>
                                    <button
                                        type="button"
                                        class="inline-flex size-8 items-center justify-center rounded-full border border-white/8 bg-[#0E1628] text-[#94A3B8]"
                                        @click="openTip = tipMap.role"
                                    >
                                        <BadgeInfo class="size-4" />
                                    </button>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    <button
                                        v-for="option in props.scout.meta.roles"
                                        :key="option"
                                        type="button"
                                        class="min-h-10 rounded-full border px-4 text-xs font-semibold"
                                        :class="row.profile.role === option ? 'border-[rgba(74,222,128,0.28)] bg-[rgba(74,222,128,0.12)] text-[#F8FAFC]' : 'border-white/6 bg-[#0E1628] text-[#94A3B8]'"
                                        :disabled="!canManage"
                                        @click="toggleChoice(row, 'role', option)"
                                    >
                                        {{ option }}
                                    </button>
                                </div>
                            ...
                            <div class="rounded-[16px] border border-white/6 bg-[#131B2F] p-4">
                                <div class="mb-3 flex items-center justify-between gap-3">
                                    <div class="flex items-center gap-2">
                                        <Sparkles class="size-4 text-[#E5B849]" />
                                        <p class="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#94A3B8]">
                                            Consistencia
                                        </p>
                                    </div>
                                    <button
                                        type="button"
                                        class="inline-flex size-8 items-center justify-center rounded-full border border-white/8 bg-[#0E1628] text-[#94A3B8]"
                                        @click="openTip = tipMap.offensive_consistency"
                                    >
                                        <BadgeInfo class="size-4" />
                                    </button>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    <button
                                        v-for="option in props.scout.meta.consistencies"
                                        :key="option"
                                        type="button"
                                        class="min-h-10 rounded-full border px-4 text-xs font-semibold"
                                        :class="row.profile.offensive_consistency === option ? 'border-[rgba(248,113,113,0.28)] bg-[rgba(248,113,113,0.12)] text-[#F8FAFC]' : 'border-white/6 bg-[#0E1628] text-[#94A3B8]'"
                                        :disabled="!canManage"
                                        @click="toggleChoice(row, 'offensive_consistency', option)"
                                    >
                                        {{ option }}
                                    </button>
                                </div>
                        ...
                        <div class="grid gap-3">
                            <div
                                ...
                                :key="field.key"
                                class="rounded-[16px] border border-white/6 bg-[#131B2F] p-4"
                            >
                                <div class="flex flex-wrap items-center justify-between gap-3">
                                    <div class="flex items-center gap-2">
                                        <component :is="field.icon" class="size-4 text-[#E5B849]" />
                                        <p class="text-[12px] font-semibold uppercase tracking-[0.18em] text-[#94A3B8]">
                                            {{ field.label }}
                                        </p>
                                    </div>
                                    <button
                                        type="button"
                                        class="inline-flex size-8 items-center justify-center rounded-full border border-white/8 bg-[#0E1628] text-[#94A3B8]"
                                        @click="openTip = tipMap[field.key]"
                                    >
                                        <BadgeInfo class="size-4" />
                                    </button>
                                </div>
                                ...
                                <div class="mt-3 flex flex-wrap items-center gap-2">
                                    <button
                                        ...
                                        :class="row.profile[field.key] >= star ? 'border-[rgba(229,184,73,0.28)] bg-[rgba(229,184,73,0.16)] text-[#E5B849]' : 'border-white/6 bg-[#0E1628] text-[#64748B]'"
                                        :disabled="!canManage"
                                        @click="toggleRating(row, field.key, star)"
                                    >
                                        <Star class="size-4" :fill="row.profile[field.key] >= star ? 'currentColor' : 'none'" />
                                    </button>
                                    <span class="ml-auto text-[12px] text-[#94A3B8]">
                                        {{ row.profile[field.key] }}/5
                                    </span>
                                </div>
                            </div>
                        </div>
        ...
        <Dialog :open="openTip !== null" @update:open="openTip = null">
            <DialogContent class="border-white/8 bg-[#1A243A] text-[#F8FAFC]">
                <DialogHeader class="space-y-4">
                    <div class="flex items-center gap-3">
                        <div class="inline-flex size-11 items-center justify-center rounded-full border border-[rgba(229,184,73,0.24)] bg-[rgba(229,184,73,0.12)] text-[#E5B849]">
                            <component :is="openTip?.icon" class="size-5" />
                        </div>
                        <DialogTitle class="app-display text-[30px]">{{ openTip?.title }}</DialogTitle>
                    </div>
                    <div
                        v-if="openTip"
                        class="text-[13px] leading-7 text-[#94A3B8] [&_strong]:text-[#F8FAFC]"
                        v-html="openTip.body_html"
                    />
                </DialogHeader>
                <DialogFooter class="flex items-center justify-between gap-3">
                    <span
                        v-if="openTip"
                        class="rounded-full border px-3 py-1 text-[11px] font-semibold"
                        :class="tipToneClass(openTip.tone)"
                    >
                        {{ openTip.tag }}
                    </span>
                    <Button
