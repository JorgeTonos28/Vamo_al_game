...
- Durante el juego se pueden registrar puntos por equipo, puntos por jugador, reversar puntos de jugador, marcar salida de jugador, ceder el turno de un jugador en cancha a alguien elegible de la cola, deshacer la ultima accion, cerrar el juego, cerrar la jornada y limpiar el juego actual.
- El mismo boton de salida en web y mobile ahora abre un flujo en pasos: elegir si el jugador se va o si cede turno, seleccionar al reemplazo desde la cola cuando aplique y confirmar la accion antes de ejecutar el cambio.
- Si un jugador sale del juego o cede su turno despues de haber anotado, sus puntos se conservan para `Stats`, `Tabla`, `Temporada` y `Scout`, pero ese juego ya no le suma victoria, derrota ni juego disputado; solo se mantiene el puntaje individual y el marcador global no se altera.
- Cuando el jugador cede turno, pasa exactamente a la posicion de cola del jugador seleccionado y el reemplazo entra a la cancha sin romper el orden operativo restante. Un participante que ya salio de ese juego no vuelve a entrar en el mismo juego aunque siga en cola.
