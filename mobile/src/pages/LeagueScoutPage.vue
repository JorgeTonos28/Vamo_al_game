...
import { BarChart3, Brain, CircleDot, Flame, Handshake, Shield, Star, Tag, Target, Zap  } from 'lucide-vue-next'
...
const ratingFields = [
  ...
  { key: 'scoring_rating', label: 'Anotación' },
...
function tipToneClass(tone: ScoutTip['tone']): string {
 return tone === 'manual' ? 'member-chip member-chip--manual' : tone === 'hybrid' ? 'member-chip member-chip--positive' : 'member-chip member-chip--warning' 
}
</script>
...
<IonRefresher  @ionRefresh="handleRefresh"><IonRefresherContent pulling-text="Desliza para refrescar" refreshing-spinner="crescent" /></IonRefresher>
</template>
      <div class="mobile-shell"><div class="mobile-stack">
        <MobileAppTopbar :title="payload?.league.name ?? 'Scout'" description="Rating híbrido, ranking y auto balance del draft." />
        ...
        <section class="app-surface section-stack">
          ...
          <p class="body-copy">{{ payload?.scout.summary.auto_preview_ready ? 'La vista previa del auto balance ya está disponible.' : 'Se necesitan 10 jugadores en pool para previsualizar el auto balance.' }}</p>
        </section>
        ...
        <section class="app-surface section-stack">
          ...
          <article v-for="(row, index) in payload?.scout.ranking.slice(0, 8) ?? []" :key="row.player.id" class="data-row"><div><p class="data-row__name">{{ ['#1','#2','#3'][index] ?? `#${index + 1}` }} / {{ row.player.name }}</p><p class="body-copy">{{ row.profile.position || 'Sin posición' }} / {{ row.profile.role || 'Sin rol' }}</p></div><span class="member-chip member-chip--warning">{{ row.combined_rating.toFixed(1) }}</span></article>
        </section>
        ...
        <section class="app-surface section-stack">
          ...
          <p v-if="!payload?.scout.auto_preview" class="body-copy">Aún no hay un pool inicial listo para simular el reparto automático.</p>
        ...
        <section class="app-surface section-stack">
          ...
          <article v-for="row in filteredPlayers" :key="row.player.id" class="player-card">
            ...
            <div v-if="row.stat_rating" class="summary-grid summary-grid--five">
              <article class="summary-card"><p class="app-kicker">Anotación</p><p class="summary-card__value summary-card__value--positive">{{ row.stat_rating.scoring.toFixed(1) }}</p><p class="body-copy">{{ row.stat_rating.detail.points_per_game.toFixed(1) }} pts/j</p></article>
            ...
            <div v-for="field in ratingFields" :key="`${row.player.id}-${field.key}`" class="rating-block">
              ...
              <p v-if="statValue(row, field.key) !== null" class="body-copy">Base estadística {{ statValue(row, field.key)?.toFixed(1) }} / {{ statDetail(row, field.key) }}</p>
            </div>
          </article>
        </section>
      </div></div>
